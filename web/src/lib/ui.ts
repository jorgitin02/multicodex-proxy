...
export function normalizeDashboardPreferences(
  ...
  return {
    tabOrder:
      Array.isArray(raw.tabOrder) && raw.tabOrder.length
        ? raw.tabOrder
        : DEFAULT_DASHBOARD_PREFERENCES.tabOrder,
    ranges: {
      overview:
        raw.ranges?.overview ?? DEFAULT_DASHBOARD_PREFERENCES.ranges.overview,
      accounts:
        raw.ranges?.accounts ?? DEFAULT_DASHBOARD_PREFERENCES.ranges.accounts,
      tracing:
        raw.ranges?.tracing ?? DEFAULT_DASHBOARD_PREFERENCES.ranges.tracing,
    },
    tracing: {
      cardOrder:
        Array.isArray(raw.tracing?.cardOrder) && raw.tracing!.cardOrder.length
          ? raw.tracing!.cardOrder
          : DEFAULT_DASHBOARD_PREFERENCES.tracing.cardOrder,
      hiddenCards: Array.isArray(raw.tracing?.hiddenCards)
        ? raw.tracing!.hiddenCards
        : [],
      graphsHidden: raw.tracing?.graphsHidden === true,
      topSessionsSort:
        raw.tracing?.topSessionsSort ?? DEFAULT_TOP_SESSIONS_SORT,
    },
    accounts: {
      sectionOrder:
        Array.isArray(raw.accounts?.sectionOrder) &&
        raw.accounts!.sectionOrder.length
          ? raw.accounts!.sectionOrder
          : DEFAULT_DASHBOARD_PREFERENCES.accounts.sectionOrder,
      hiddenSections: Array.isArray(raw.accounts?.hiddenSections)
        ? raw.accounts!.hiddenSections
        : [],
    },
  };
}
