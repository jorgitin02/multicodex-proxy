...
import type { ExposedModel, TraceRangePreset, TraceStats, TraceUsageStats } from "../../types";
...
export function OverviewTab({
  ...
  const filteredModels = useMemo(() => {
    ...
    return models.filter((model) => (model.metadata?.provider ?? "openai") === providerTab);
  }, [models, providerTab]);
      ...
      <section className="panel">
        <div className="inline wrap row-between">
          <div>
            ...
            <p className="muted">Trace-window analytics update with the selected time range.</p>
          </div>
      ...
      <section className="panel">
        <div className="inline wrap row-between">
          <div>
            ...
            <p className="muted">These reflect provider-reported quota windows, not the selected trace range.</p>
          </div>
          <span className="muted">Last refresh {fmt(providerQuotaStats.freshestAt)}</span>
        </div>
        <div className="grid cards2">
          <div>
            <ProgressStat label="5h average" value={providerQuotaStats.primaryAvg} count={providerQuotaStats.primaryCount} />
            <ProgressStat label="Weekly average" value={providerQuotaStats.secondaryAvg} count={providerQuotaStats.secondaryCount} />
          </div>
          <ul className="clean-list">
            ...
            <li>Provider snapshot unsupported: {providerQuotaStats.unsupported}</li>
          </ul>
        </div>
      </section>
        ...
        <section className="panel">
          ...
          {storageInfo && (
            <ul className="clean-list">
              ...
              <li className="mono">trace stats: {storageInfo.traceStatsHistoryPath}</li>
        ...
        <section className="panel">
          ...
          <div className="chips">
            {filteredModels.map((model) => (
              <span key={model.id} className="chip mono">{model.id}</span>
            ))}
          </div>
        </section>
      </section>
